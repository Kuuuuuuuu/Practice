...
class KnockbackManager
{
    ...
    public function setKnockback(Player $player, string $world, float|int $knockback1, float|int $knockback2): void
    {
        if (!file_exists(Server::getInstance()->getDataPath() . 'worlds/' . $world)) {
            $player->sendMessage(PracticeCore::getPrefixCore() . Color::RED . 'World ' . $world . ' not found');
        } else {
            ...
            $data->save();
            $player->sendMessage(PracticeCore::getPrefixCore() . Color::GREEN . 'Knockback set to ' . $knockback1 . ' for world ' . $world);
        }
    }
    ...
    public function setAttackspeed(Player $player, string $world, int $speed): void
    {
        if (!file_exists(Server::getInstance()->getDataPath() . 'worlds/' . $world)) {
            ...
            $data->save();
            $player->sendMessage(PracticeCore::getPrefixCore() . Color::GREEN . 'Attackspeed set to ' . $speed . ' for world ' . $world);
        }
    }
    ...
    public function removeAttackspeed(Player $player, string $world): void
    {
        ...
        $data->save();
        $player->sendMessage(PracticeCore::getPrefixCore() . Color::GREEN . 'Attackspeed removed for world ' . $world);
    }
    ...
    public function removeKnockback(Player $player, string $world): void
    {
        ...
        $data->remove(mb_strtolower($world));
        $data->save();
        $player->sendMessage(PracticeCore::getPrefixCore() . Color::GREEN . 'Knockback removed for world ' . $world);
    }
}