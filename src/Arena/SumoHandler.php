...
class SumoHandler implements Listener
{
    ...
    {
        if (!$this->data["enabled"]) {
            $player->sendMessage(Loader::getInstance()->getPrefixCore() . "§e The game is in configurating!");
            return;
        }
        if (count($this->players) >= $this->data["slots"]) {
            $player->sendMessage(Loader::getInstance()->getPrefixCore() . "§eThe game is full!");
            return;
        }
        if ($this->inGame($player)) {
            $player->sendMessage(Loader::getInstance()->getPrefixCore() . "You are already in the queue/game!");
            return;
        }
        $selected = false;
    ...
    public function startRestart()
    {
        ...
        $this->plugin->getServer()->broadcastMessage(Loader::getInstance()->getPrefixCore() . "§r§ePlayer {$player->getName()} won the Sumo!");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "You left the Game")
    {
        ...
        $player->getInventory()->clearAll();
        $player->getArmorInventory()->clearAll();
        Loader::getInstance()->inSumo[$player->getName()] = false;
        ...
        $player->sendMessage(Loader::getInstance()->getPrefixCore() . "§r§e$quitMsg");
    }
